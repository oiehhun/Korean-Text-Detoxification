...
class SHAP:
    def __init__(self,tokenizer,model) -> None:
          ...
          with open('./datasets/stopwords.txt', 'r', encoding='utf-8') as file:
            # 파일 내용을 줄 단위로 읽어와 리스트에 저장
            lines = file.readlines()
            self.stopwords = [line.strip() for line in lines]

    def f(self,x):
        tv = torch.tensor([self.tokenizer.encode(v, pad_to_max_length=True, max_length=128, truncation=True) for v in x])
